...
function Cart() {
  ...
  useEffect(() => {
    let count = 0;
    cartItems.forEach((cartItem) => { count = count + parseInt(cartItem.price.split('').filter(i=>i!=="," && i!=="₹").join(""))
      console.log(cartItem.price.split('').filter(i=>i!=="," && i!=="₹").join(""))
    })
  ...
  return (
    <Layout >
      <div className="h-screen  pt-5  mb-[100%]" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
